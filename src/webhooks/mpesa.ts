...
export const mpesaWebhook = new LuaWebhook({
    ...
    execute: async (event) => {
        try {
            ...
            const anomaly: any = anomalyRes.ok ? await anomalyRes.json() : null;
            const profile: any = await profileRes.json();
            ...
            if (owner) {
                ...
                const messageText = await AI.generate(systemPrompt, userPrompt);
            ...
            if (promoEnabled && CustomerPhone && CustomerPhone !== OWNER_USER_ID) {
                try {
                    const client = await User.get(CustomerPhone);
                    if (client) {
                        await client.send([{ 
                            type: 'text', 
